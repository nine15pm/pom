"""SU (Speech Understanding) trainer."""
...
def _ensure_output_dir(path: str) -> Path:
    ...
    # Explicit write test to fail fast on permission errors.
...
def _ensure_pad_token(tokenizer) -> None:
    ...
    # Keep padding semantics separate from EOS/turn-end semantics.
    if tokenizer.pad_token_id is None:
        raise ValueError("tokenizer must define pad_token_id")


def _freeze_whisper(model) -> None:
    """Freeze the speech encoder and keep it in eval mode."""
    encoder = model.get_speech_encoder()
    if encoder is None:
        raise ValueError("speech encoder is missing on the model")
    # Keep Whisper in eval mode to disable dropout while the rest trains.
    encoder.eval()
...
def _drop_none(data: Dict[str, Any]) -> Dict[str, Any]:
    """Drop None values so defaults can apply."""
    # Keep the builder surface clean by skipping unset fields.
...
def _make_sample_id_logger(
    ...
    def _log(sample_ids):
        """Append a few sample ids for multi-GPU sharding validation."""
        nonlocal seen
...
def _build_model_and_tokenizer(cfg: Dict[str, Any]) -> tuple[torch.nn.Module, Any, TokenIds]:
    """Load the pom model + tokenizer with speech modules wired."""
    model_cfg = _get_cfg_section(cfg, "model")

    # Build the speech spec from config; omit None to use defaults.
    ...
    # Disable kv-cache for training to avoid unnecessary memory.
    model.config.use_cache = False
    _freeze_whisper(model)
...
def _build_dataloader(
    ...
    *,
    ...
    shuffle_epoch: int = 0,
) -> tuple[DataLoader, SuDataset]:
    """Construct the SU dataset and dataloader."""
    ...
    # Pass startup-resolved token ids so collator never re-derives token ids.
...
def _parse_args() -> argparse.Namespace:
    """Parse CLI flags for config and path overrides."""
    parser = argparse.ArgumentParser(description="SU trainer")
...
def _load_trainer_state(checkpoint_dir: Path) -> Dict[str, Any]:
    """Load trainer state metadata stored with each checkpoint."""
...
def _build_resume_compat(cfg: Dict[str, Any], *, grad_accum: int, precision: str) -> Dict[str, Any]:
    """Build a compact compatibility fingerprint for resume safety."""
    model_cfg = _get_cfg_section(cfg, "model")
    data_cfg = _get_cfg_section(cfg, "data")
    train_cfg = _get_cfg_section(cfg, "training")
    return {
        ...
        # Keep scheduler-defining fields stable across resume.
        "training_steps": int(train_cfg.get("steps", 0)),
        "warmup_ratio": float(train_cfg.get("warmup_ratio", 0.03)),
        "lr": float(train_cfg.get("lr", 5.0e-5)),
        "max_grad_norm": float(train_cfg.get("max_grad_norm", 1.0)),
    }


def _validate_resume_compat(resume_state: Dict[str, Any], current_compat: Dict[str, Any]) -> None:
    """Fail fast if a checkpoint comes from an incompatible run config."""
...
def _save_checkpoint(
    *,
    ...
    """Save accelerator state and tiny trainer metadata for resume."""
    ...
    if accelerator.is_main_process:
        # Keep the resume state explicit and easy to inspect.
        trainer_state = {
        ...
        # Retain only the newest checkpoints; 0 means keep current checkpoint only.
        root = checkpoint_dir.parent
        checkpoints = sorted(path for path in root.glob("step-*") if path.is_dir())
...
def main() -> None:
    """Entry point: load config, build trainer, and run."""
    args = _parse_args()
    ...
    # 1) Accelerator owns device placement, precision, and DDP wiring.
    grad_accum = int(train_cfg.get("grad_accum", 1))
    precision = str(train_cfg.get("precision", "bf16")).lower()
    # Project policy: only bf16 or full precision ("no") on Ada GPUs.
    ...
    # Control IterableDataset sharding and dataloader transfer behavior (Accelerate v1.12+).
    ...
    # 2) Set a global seed to improve reproducibility.
    seed = train_cfg.get("seed")
    if seed is not None:
        # Use Accelerate's global seed helper for DDP-safe RNG seeding.
        set_seed(int(seed))

    # 3) Build the model/tokenizer and the data pipeline.
    ...
    if data_is_sharded:
        ...
        # Single-file datasets always start from shard 0.
        resume_next_shard_cursor = 0
    ...
    # 4) Optimizer + scheduler are step-based (we always require steps).
    lr = float(train_cfg.get("lr", 5.0e-5))
    max_grad_norm = float(train_cfg.get("max_grad_norm", 1.0))
    if max_grad_norm <= 0:
    ...
    if total_steps is None:
        raise ValueError("training.steps must be set for SU training")
    ...
    # 5) Prepare for distributed training (DDP, fp16/bf16, etc.).
    model, optimizer, dataloader = accelerator.prepare(model, optimizer, dataloader)
    # Reuse optimizer-owned params for clipping to match optimizer stepping exactly.
    optim_params = [param for group in optimizer.param_groups for param in group["params"]]
    # Keep scheduler unwrapped so it advances exactly once per optimizer step.
    accelerator.register_for_checkpointing(scheduler)
    if resume_dir is not None:
        # Restore model/optimizer/scheduler/device state before stepping.
        accelerator.load_state(str(resume_dir))

    # 6) Enter train mode but keep Whisper frozen and in eval mode.
    model.train()
    _freeze_whisper(accelerator.unwrap_model(model))
    ...
    if wandb_cfg.get("enabled", False):
        ...
        if not wandb_run_id:
            # Generate a stable id so checkpoints can reconnect on future resumes.
            wandb_run_id = uuid.uuid4().hex

    # 7) Init wandb tracker only when requested.
    if wandb_cfg.get("enabled", False):
    ...
    # 8) Fail-fast: ensure one batch works and loss is valid.
    ...
    batch.pop("shard_indices", None)
    batch.pop("shard_cursors", None)
    batch.pop("worker_ids", None)
    with torch.no_grad():
    ...
    # 9) Train for a fixed number of optimizer steps.
    step = resume_step
    next_shard_cursor = resume_next_shard_cursor if data_is_sharded else 0
    shuffle_epoch = resume_shuffle_epoch
    # Track per-worker progress so resume is conservative and skip-free.
    num_workers = int(train_cfg.get("num_workers", 0))
    ...
    # Keep step-window logging accumulators on device to avoid per-micro sync overhead.
    ...
    while step < total_steps:
        ...
        try:
            if profile_enabled:
                # Measure how long we wait for the next batch from the dataloader.
            ...
            # Restart the iterator when the dataloader is exhausted.
            if data_is_sharded:
                # Reset shard cursor and advance epoch when shuffling is enabled.
                next_shard_cursor = 0
                dataset.start_shard_cursor = 0
                # Reset per-worker progress at the start of a new epoch.
                last_cursor_seen = {
                    worker_id: int(next_shard_cursor) for worker_id in range(worker_count)
                }
                if shuffle_shards:
        ...
        # Count consumed micro-batches for resume metadata.
        ...
        if data_is_sharded and shard_cursors and worker_ids:
            # Update per-worker last-seen cursor from this batch.
            ...
            # Resume at the minimum cursor seen across workers/ranks (conservative).
        ...
        with accelerator.accumulate(model):
            ...
            target_tokens_micro = (labels != -100).sum()
            # Track numerator/denominator so train/loss matches the full optimizer step.
            ...
            if accelerator.sync_gradients:
                # Clip once per optimizer step and log the returned total grad norm.
                ...
                optimizer.step()
                # Advance LR once per optimizer step.
                scheduler.step()
                optimizer.zero_grad()
                step += 1

                # Compute global token-weighted loss for this optimizer step window.
                ...
                # Start fresh accumulators for the next optimizer step window.
                ...
                # Next loop iteration starts a new accumulation window.

    # 10) Always save a final checkpoint.
    _save_checkpoint(
        accelerator=accelerator,
        output_dir=output_dir,
        step=step,
